...
bool ofxPJControl::getProjectorStatus() { return projStatus; }
...
void ofxPJControl::setup(string IP_add, int port, int protocol,
                         string password) {
...
void ofxPJControl::setProjectorIP(string IP_add) { IPAddress = IP_add; }

void ofxPJControl::setProjectorPort(int port) { pjPort = port; }

void ofxPJControl::setProjectorPassword(string passwd) { password = passwd; }
...
bool ofxPJControl::sendPJLinkCommand(string command) {
  ...
  if (!pjClient.isConnected()) {
    ...
    if (connected) {
      ofLogNotice("", "connection established to %s:%i", IPAddress.c_str(),
                  pjPort);
      string response = "";
  ...
  if (connected) {
    ...
    msgRx = msgRx.substr(command.length() - 2,
                         msgRx.length() - command.length() + 1);
...
bool ofxPJControl::sendCommand(string command) {
  ...
  ofLogNotice() << "Response length (Bytes) : "
                << pjClient.getNumReceivedBytes() << endl;
  msgRx = "";
  msgRx = pjClient.receiveRaw();
...
bool ofxPJControl::nec_On() {
  ...
  char* buffer = new char[6];  // 02H 00H 00H 00H 00H 02H (the on command in
                               // hex)
  buffer[0] = 2;
  buffer[1] = 0;
  buffer[2] = 0;
...
bool ofxPJControl::nec_Off() {
  char* buffer =
      new char[6];  // 02H 01H 00H 00H 00H 03H (the off command in hex)
  buffer[0] = 2;
  buffer[1] = 1;
  buffer[2] = 0;
  buffer[3] = 0;
...
string ofxPJControl::getPJLinkResponse() { return msgRx; }
bool ofxPJControl::isConnected() { return connected; }