...
class ofxPJControl {
 public:
  ...
  bool sendPJLinkCommand(
      std::string command);  // send any PJLink command to the projector
  void setup(std::string IP_add = "192.168.0.100", int port = 4352,
             int protocol = PJLINK_MODE, std::string password = "");  // default
  ...
  void setProjectorPassword(
      std::string passwd);  // password for PJLink authentication
  bool
  getProjectorStatus();  // return whether projector is on (true) or off (false)
  void setProjectorPort(int port);  // the network port of the projector
  bool sendCommand(
      std::string command);  // send any string command to the projector without
                             // password authentication

  std::string getPJLinkResponse();  // return the response from the projector
  bool isConnected();               // return whether the projector is connected
